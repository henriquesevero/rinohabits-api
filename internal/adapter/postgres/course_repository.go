...
func (r CourseRepository) Create(ctx context.Context, c *course.Course) error {
	_, err := r.pool.Exec(ctx,
		`INSERT INTO courses (id, user_id, title, description, link, status, total_hours, current_hours, cover_url, started_at, finished_at, sort_order)
...
func (r CourseRepository) FindByID(ctx context.Context, id uuid.UUID) (*course.Course, error) {
	row := r.pool.QueryRow(ctx,
		`SELECT id, user_id, title, description, link, status, total_hours, current_hours, sort_order, cover_url, started_at, finished_at, created_at, updated_at
...
func scanCourse(row rowScanner) (*course.Course, error) {
	...
	err := row.Scan(
		...
		&c.TotalHours, &c.CurrentHours, &c.SortOrder, &c.CoverURL,
