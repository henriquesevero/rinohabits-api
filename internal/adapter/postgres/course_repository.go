...
func (r CourseRepository) Create(ctx context.Context, c *course.Course) error {
	_, err := r.pool.Exec(ctx,
		`INSERT INTO courses (id, user_id, title, description, link, status, total_hours, current_hours, cover_url, started_at, finished_at, collection, active_weekdays, reminder_hour, reminder_minute, sort_order)
		...
		toInt16Slice(c.ActiveWeekdays), c.ReminderHour, c.ReminderMinute,
	)
	return err
}
...
func (r CourseRepository) FindByID(ctx context.Context, id uuid.UUID) (*course.Course, error) {
	row := r.pool.QueryRow(ctx,
		`SELECT id, user_id, title, description, link, status, total_hours, current_hours, sort_order, collection, cover_url, started_at, finished_at, active_weekdays, reminder_hour, reminder_minute, created_at, updated_at
...
func (r CourseRepository) Update(ctx context.Context, c *course.Course) error {
	_, err := r.pool.Exec(ctx,
		`UPDATE courses SET title=$2, description=$3, link=$4, status=$5, total_hours=$6, current_hours=$7, cover_url=$8, started_at=$9, finished_at=$10, collection=$11, active_weekdays=$12, reminder_hour=$13, reminder_minute=$14
		 WHERE id=$1`,
...
func (r CourseRepository) DeleteAllByUser(ctx context.Context, userID uuid.UUID) error {
	_, err := r.pool.Exec(ctx, `DELETE FROM courses WHERE user_id = $1`, userID)
	return err
}
...
func scanCourse(row rowScanner) (*course.Course, error) {
	...
	err := row.Scan(
		...
		&c.StartedAt, &c.FinishedAt, &weekdays, &c.ReminderHour, &c.ReminderMinute,
	...
	c.ActiveWeekdays = toIntSlice(weekdays)
	return &c, nil
}
